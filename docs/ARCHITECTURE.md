...
2. **Transactions:** `[AppData]/trips/[TripSlug]/transactions/[TransactionID]/`
    ...
    - **Structure:**
        - `details_versioned/`: Versioned folder containing `transaction_details.json`.
...
2.  **Integrity & Conflict Check:**
    *   Invoke the `VersionedStorageEngine` for the `config_versioned/` folder and each folder in `transactions/`.
...
Node folders (folders containing other folders, like `config_versioned/` or `transactions/`) are traversed recursively and do not use the `.synching.tf` logic themselves; the logic applies to their descendant leaf folders.
